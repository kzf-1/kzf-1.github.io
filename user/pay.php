    ...
    <div class="row">
                    ...
                    <form class="form-horizontal layui-form">
                        ...
                        <div class="form-group mb-2">
                            ...
                            <select class="custom-select mt-3" name="type">
                                ...
                                <option value="alipay"
                                    <?= ($withdrawal['type'] == 'alipay' ? ' selected ' : '') . ($type[0] <> 1 ? 'disabled' : '') ?>
                                >支付宝收款 <?= ($type[0] <> 1 ? ' - 暂不支持' : '') ?></option>
                                <option value="wxpay"
                                    <?= ($withdrawal['type'] == 'wxpay' || $withdrawal['type'] == 'wxpai' ? ' selected ' : '') . ($type[1] <> 1 ? 'disabled' : '') ?>
                                >微信收款 <?= ($type[1] <> 1 ? ' - 暂不支持' : '') ?></option>
                                <option value="qqpay"
                                    <?= ($withdrawal['type'] == 'qqpay' ? ' selected ' : '') . ($type[2] <> 1 ? 'disabled' : '') ?>
                                >QQ收款 <?= ($type[2] <> 1 ? ' - 暂不支持' : '') ?></option>
                            </select>
                        </div>
        ...
        <div class="col-xl-6">
            <div class="card">
                <div class="card-header">
                    提现日志
                    <span class="badge badge-primary-lighten">待处理(<?= $DB->sum('withdrawal', 'money', ['uid' => $UserData['id'], 'state' => 3]) ?>元)</span>
                    <span class="badge badge-warning-lighten">累计提现(<?= $DB->sum('withdrawal', 'money', ['uid' => $UserData['id']]) ?>元) </span>
                    <span class="badge badge-success-lighten">成功提现(<?= $DB->sum('withdrawal', 'money', ['uid' => $UserData['id'], 'state' => 1]) ?>元)</span>
                </div>
                <div class="card-body">
