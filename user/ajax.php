...
switch ($_QET['act']) {
    ...
    case 'login_register': //用户注册！
        ...
        if (empty((int)$_QET['qq']) || empty($_QET['name']) || empty((int)$_QET['mobile']) || empty($_QET['username']) || empty($_QET['password']) || empty($_QET['vercode'])) {
            dies(-1, '请填写完整！');
        }
    ...
    case 'user_pay':
        ...
        if ($Money < 0.01 || $Money > 2000) {
            dies(-1, '最少充值0.01元，最多充值2000元，若要充值大额，可联系客服或分多次进行充值！');
        }
    ...
    case 'configuration_save':
        ...
        switch ($_QET['type']) {
            ...
            case 'webset':
                ...
                if ((int)$_QET['award'] < 0 || $_QET['Signin'] < 0) dies(-1, '请填写完整!');
                $award = $DB->get('config', '*', ['K' => 'award'])['V'];
                $Signin = $DB->get('config', '*', ['K' => 'Signin'])['V'];
                if ((int)$award < 0 || (int)$Signin < 0) dies(-1, '主站数据获取失败！');
