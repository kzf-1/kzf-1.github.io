...
switch ($_QET['act']) {
    case 'InstallData':
        ...
        break;
    case 'Install': //安装程序
        test(['host', 'port', 'dbname', 'pwd', 'token', 'type', 'user', 'versions'], '请提交完整参数！');
        if (file_exists('install.lock')) {
        ...
        dies(($success !== 0 ? 1 : -1), '安装结果如下：<br>数据写入成功数：' . $success . '条<br>失败数：' . $error . '条<br>点击下一步继续！' . (empty($content) ? '' : '<br>' . $content));
        break;
    case 'Ping': //节点Ping
        test(['id']);
        Curl::Ping($_QET['id']);
