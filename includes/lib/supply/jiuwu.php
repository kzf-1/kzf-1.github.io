...
class jiuwu
{
    ...
    public static function TakeOutTheData($data)
    {
        ...
        if (empty($tabs) || empty($tabs[0])) {
            dies(-1, '数据获取失败,无法打开对应页面：' . $CurlHtml);
        }
        if (!$tabs || count($tabs) <= 0) {
            dies(-1, '商品下单数据获取失败，请联系官方处理，需提供对接的社区和商品信息');
        }
        if (empty($tabs[1])) {
            $HtmlDataT2 = $tabs[0];
        } else {
        ...
        preg_match('/现金:<span class="user_rmb">(.*?)<\/span>元/', $HtmlData, $balance);
        if ($balance[1]) {
            //取账户余额
            $Money = $balance[1];
        } else {
    ...
    public static function ProductDetails($Data)
    {
        ...
        if ($Goods['goods_status'] === 1) {
            //一般是业务维护,无法下单的商品,允许先上架,后续监控时,会自动下架
            /*return [
                'code' => -1,
                'msg' => $Goods['goods_close_msg'],
            ];*/
        }
        ...
        return [
            'code' => 1,
            'msg' => '对接参数获取成功！' . ($His['balance'] ? '<br>当前账户余额：<font color=red>' . $His['balance'] . '元</font>' : ''),
            'data' => [
                'name' => $Goods['title'],
                'image' => $Goods['thumb'],
                'docs' => $His['docs'],
