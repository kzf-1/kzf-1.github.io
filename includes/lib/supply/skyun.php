...
class skyun
{
    ...
    public static function Query($id, $TypeSupply)
    {
        ...
        $TypeSupply['url'] = StringCargo::UrlVerify($TypeSupply['url'], 2);
        $DataCurl = Api::Curl('http://' . $TypeSupply['url'] . '.skysq.top/api/user/order/query', $DataPost);
        if (empty($DataCurl)) {
            return false;
        }
    ...
    public static function Submit($OrderData, $Goods, $TypeSupply)
    {
        ...
        $DataCurl = Api::SuppluCurl('http://' . $TypeSupply['url'] . '.skysq.top/api/user/order', $DataPost);
         ...
         * 卡密转换
         */
        if (isset($DataCurl['type']) && (int)$DataCurl['type'] === 2) {
            $SQL = [];
            foreach ($DataCurl['kms'] as $v) {
                if (empty($v)) {
                    continue;
                }
                $SQL[] = [
    ...
    public static function CommodityStatus($Goods, $Supply)
    {
        ...
        $Supply['url'] = StringCargo::UrlVerify($Supply['url'], 2);
        ...
        $CurlData = Api::Curl('http://' . $Supply['url'] . '.skysq.top/api/user/goods/info', $Post);
    ...
    public static function ProductDetails($Data)
    {
        ...
        $CurlData = Api::Curl('http://' . $SourceData['url'] . '.skysq.top/api/user/goods/info', $Post);
        ...
        if ($CurlDataJson['status'] === 0) {
            ...
            if ((!empty($Goods['image_url']) && strpos($Goods['image_url'], 'http') === false)) {
                $Goods['image_url'] = 'http://' . $SourceData['url'] . '/' . $Goods['image_url'];
            }
    ...
    public static function GoodsList($Data)
    {
        ...
        $CurlData = Api::Curl('http://' . $SourceData['url'] . '.skysq.top/api/user/goods/list', $Post);
